...
int embedding(const char** c_res_prompts, int prompts_count, float*** res_out, int* res_sizes, int *vector_size,const char* c_model_path,int32_t user_n_batch,int total_elements,int layers){
   ...
   for(int i=0;i<prompts_count;i++){
      ...
      res_prompts.emplace_back(std::string(c_res_prompts[i]));
   }
  ...
  res.reserve(8000);
  std::string str_layers = std::to_string(layers);
   int argc = 9;
   char *argcv[10];
   argcv[0] = "";
   argcv[1] = "-m";
   ...
   argcv[7] = "-ngl";
   argcv[8] = const_cast<char*>(str_layers.c_str());;
   argcv[9] = nullptr;
    ...
    for(auto &res_prompt:res_prompts){   //对于每一个prompt 进行处理
       ...
       /// std::vector<std::string> prompts;
        ...
        for (const auto & prompt : prompts) {
            ...
            auto inp = ::llama_tokenize(ctx, prompt, true, false);  //对每个prompt 进行tokenize
            std::cout<<"token size: "<<inp.size()<<std::endl;
            if(inp.empty()){
                std::cerr << "Tokenization failed for prompt: " << prompt << std::endl;
            }
            std::cout<<"3"<<std::endl;
            if (inp.size() > n_batch) {
        ...
        std::vector<float> embeddings(n_prompts * n_embd, 0); //初始分配了4096*n_prompts大小的向量 初始化为0
        float * emb = embeddings.data(); //emb指向它
        std::cout<<"9"<<std::endl;

        // break into batches
        int p = 0; // number of prompts processed already
        int s = 0; // number of prompts in current batch
        for (int k = 0; k < n_prompts; k++) {  //对于按行分割后的prompt处理
            // clamp to n_batch tokens
            auto & inp = inputs[k];  //每个prompt对应的token
        ...
        if (params.embd_out.empty()) {
            ...
            //int total_elements = 768;
            //int total_elements = 512;
            int elements_per_batch = total_elements / n_prompts;
            int remaining_elements = total_elements % n_prompts;

            for (int j = 0; j < n_prompts; j++) {  //n_prompts = 1
                int elements_this_batch = elements_per_batch + (j < remaining_elements ? 1 : 0);
                for (int i = 0; i < elements_this_batch; i++) {
                    //int index = j * n_embd + i * (n_embd / elements_this_batch);
                    //int index = j * n_embd + i * (n_embd / std::max(elements_this_batch, 1));
                    int index = j * n_embd + i;
                    temp.push_back(emb[index]);
                }
            }
